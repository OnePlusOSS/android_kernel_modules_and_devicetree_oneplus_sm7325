...
static ssize_t proc_ui_soc_decimal_read(struct file *filp, char __user *buff, size_t count, loff_t *off)
{
	...
	int schedule_work = 0;
	int val;
	int chg_fast_type = CHARGER_SUBTYPE_DEFAULT;
	bool svooc_is_control_by_vooc = false;
	...
	if (chip->vooc_show_ui_soc_decimal) {
		...
		svooc_is_control_by_vooc = (chg_fast_type == CHARGER_SUBTYPE_FASTCHG_VOOC);
		control_by_wireless =
		...
		if (svooc_is_control_by_vooc != true && chip->calculate_decimal_time == 0 &&
		    ...
		    oplus_chg_show_vooc_logo_ornot() == true && chg_fast_type != CHARGER_SUBTYPE_DEFAULT) {
